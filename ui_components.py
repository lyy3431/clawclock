...
class UIMixin:
    ...
    def _setup_timezone_selector(self, parent: tk.Frame) -> None:
        """创建时区选择器 UI"""
        tz_frame: tk.Frame = self.tk.Frame(parent, bg=self.bg_color)
        ...
        self.tz_combo = self.ttk.Combobox(tz_frame, values=tz_values, width=28, state="readonly")
    ...
    def _setup_theme_selector(self, parent: tk.Frame) -> None:
        """创建主题选择器 UI"""
        theme_frame: tk.Frame = self.tk.Frame(parent, bg=self.bg_color)
        ...
        self.theme_combo = self.ttk.Combobox(theme_frame, values=theme_values, width=28, state="readonly")
    ...
    def _setup_mode_selector(self, parent: tk.Frame) -> None:
        """创建模式选择器 UI"""
        self.mode_var = self.tk.StringVar(value=self.display_mode)
        mode_frame: tk.Frame = self.tk.Frame(parent, bg=self.bg_color)
    ...
    def _setup_action_buttons(self, parent: tk.Frame) -> None:
        """创建功能按钮 UI"""
        btn_frame: tk.Frame = self.tk.Frame(parent, bg=self.bg_color)
        ...
        self.fullscreen_btn = self.tk.Button(btn_frame, text="🔲 全屏", command=self.toggle_fullscreen,
                                                   bg=self.accent_color, fg=self.text_color)
        self.fullscreen_btn.pack(side=self.tk.LEFT, padx=5)

        self.topmost_btn = self.tk.Button(btn_frame, text="📌 置顶", command=self.toggle_topmost,
                                                bg=self.accent_color, fg=self.text_color)
    ...
    def _setup_canvas(self, parent: tk.Frame) -> None:
        """创建模拟时钟画布"""
        self.canvas = self.tk.Canvas(parent, width=300, height=300, bg=self.bg_color, highlightthickness=0)
        self.canvas.pack(pady=10)

    def _setup_digital_display(self, parent: tk.Frame) -> None:
        """创建数字时钟显示 UI"""
        self.digital_frame = self.tk.Frame(parent, bg=self.bg_color)

        canvas_width: int = 560
        canvas_height: int = 130
        self.seg_canvas = self.tk.Canvas(self.digital_frame, width=canvas_width, height=canvas_height,
                                               bg=self.bg_color, highlightthickness=0)
        self.seg_canvas.pack(expand=True)

        self.date_label = self.tk.Label(self.digital_frame, text="", font=("Arial", 14),
                                              bg=self.bg_color, fg=self.text_color)
        self.date_label.pack(pady=10)

    def _setup_stopwatch_ui(self, parent: tk.Frame) -> None:
        """创建秒表 UI"""
        self.stopwatch_frame = self.tk.Frame(parent, bg=self.bg_color)

        # 秒表时间显示
        self.stopwatch_time_var = self.tk.StringVar(value="00:00:00.00")
        self.stopwatch_label = self.tk.Label(self.stopwatch_frame, textvariable=self.stopwatch_time_var,
        ...
        # 秒表按钮
        sw_btn_frame: tk.Frame = self.tk.Frame(self.stopwatch_frame, bg=self.bg_color)
        sw_btn_frame.pack(pady=10)

        self.sw_start_btn = self.tk.Button(sw_btn_frame, text="▶️ 开始", command=self.toggle_stopwatch,
                                                 bg=self.accent_color, fg=self.text_color, font=("Arial", 12), width=10)
        self.sw_start_btn.pack(side=self.tk.LEFT, padx=10)

        self.sw_stop_btn = self.tk.Button(sw_btn_frame, text="⏸️ 停止", command=self.toggle_stopwatch,
                                                bg=self.accent_color, fg=self.text_color, font=("Arial", 12), width=10)
        self.sw_stop_btn.pack(side=self.tk.LEFT, padx=10)
        self.sw_stop_btn.config(state=self.tk.DISABLED)

        self.sw_lap_btn = self.tk.Button(sw_btn_frame, text="🏁 计次", command=self.record_lap,
                                               bg=self.accent_color, fg=self.text_color, font=("Arial", 12), width=10)
        ...
        # 计次记录列表
        self.lap_listbox = self.tk.Listbox(self.stopwatch_frame, font=("Courier New", 12),
    ...
    def _setup_timer_ui(self, parent: tk.Frame) -> None:
        """创建倒计时 UI"""
        self.timer_frame = self.tk.Frame(parent, bg=self.bg_color)

        # 倒计时时间显示
        self.timer_time_var = self.tk.StringVar(value="00:00:00")
        self.timer_label = self.tk.Label(self.timer_frame, textvariable=self.timer_time_var,
                                               font=("Courier New", 72, "bold"), bg=self.bg_color, fg=self.text_color)
        self.timer_label.pack(pady=20)

        # 倒计时状态标签
        self.timer_status_var = self.tk.StringVar(value="准备就绪")
        self.timer_status_label = self.tk.Label(self.timer_frame, textvariable=self.timer_status_var,
    ...
    def _setup_timer_presets(self) -> None:
        """创建倒计时预设按钮"""
        timer_preset_frame: tk.Frame = self.tk.Frame(self.timer_frame, bg=self.bg_color)
        ...
        for text, seconds, name in preset_configs:
            btn: tk.Button = self.tk.Button(timer_preset_frame, text=text,
    ...
    def _setup_timer_custom_input(self) -> None:
        """创建倒计时自定义输入 UI"""
        custom_frame: tk.Frame = self.tk.Frame(self.timer_frame, bg=self.bg_color)
        custom_frame.pack(pady=10)

        self.tk.Label(custom_frame, text="自定义:", bg=self.bg_color, fg=self.text_color).pack(side=self.tk.LEFT)

        self.timer_hour_entry = self.tk.Entry(custom_frame, width=4, justify='center')
        ...
        self.timer_min_entry = self.tk.Entry(custom_frame, width=4, justify='center')
        self.timer_min_entry.insert(0, "00")
        self.timer_min_entry.pack(side=self.tk.LEFT, padx=2)
        self.tk.Label(custom_frame, text=":", bg=self.bg_color, fg=self.text_color).pack(side=self.tk.LEFT)

        self.timer_sec_entry = self.tk.Entry(custom_frame, width=4, justify='center')
    ...
    def _setup_timer_buttons(self) -> None:
        """创建倒计时控制按钮"""
        timer_btn_frame: tk.Frame = self.tk.Frame(self.timer_frame, bg=self.bg_color)
        timer_btn_frame.pack(pady=15)

        self.timer_start_btn = self.tk.Button(timer_btn_frame, text="▶️ 开始", command=self.toggle_timer,
                                                    bg=self.accent_color, fg=self.text_color, font=("Arial", 12), width=10)
        ...
        # 声音开关
        self.timer_sound_var = self.tk.BooleanVar(value=True)
    ...
    def setup_ui(self) -> None:
        """设置用户界面"""
        # Main frame
        main_frame: tk.Frame = self.tk.Frame(self.root, bg=self.bg_color)
